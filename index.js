...
function express() {
    ...
    app.handle = function(req, res, next2) {
        ...
        function next(error) {
            ...
            for (; stackIndex < app.stack.length; stackIndex++) {
                ...
                if ((isErrorHandler(target.handle) === Boolean(error)) && (m = target.match(req.url))) {
    ...
    app.route = function(path) {
        var route = makeRoute()
        app.stack.push(new Layer(path, route, {
            end: true
        }));
        return route;
    };
            ...
            proto.sendfile = function(file_path, options) {
                ...
                (function range_request() {
                    ...
                    if (!res.req.headers['range']) {
                        ...
                        var r = rparser(stats.size, res.req.headers['range']);
                        if (r == -1) { // range not valid
                            res.statusCode = 416;
                            res.end();
                        } else if (r == -2) {
                            stream = fs.createReadStream(file_path);
                        } else {
                            res.setHeader('Content-Length', r[0].end - r[0].start + 1);
                            res.setHeader('Content-Range', r.type + ' ' + r[0].start + '-' + r[0].end + '/' + stats.size);
                            res.setHeader('Accept-Range', r[1]);
                            res.statusCode = 206;
                            stream = fs.createReadStream(file_path, {
                                start: r[0].start,
                                end: r[0].end
